...
import { generarHash } from './utils/hash';
...
generarDatosEjemplo()
  ...
  .finally(async () => {
    await prisma.$disconnect(); // <-- OBLIGATORIO para soltar la conexión
    process.exit(0);            // <-- OBLIGATORIO para que Render sepa que terminó
  });